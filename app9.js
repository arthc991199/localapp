...
// Ustawienie godzin na 00:00:00 dla porównania tylko dat
...
let timeMessage = ""; // Wiadomość o czasie do wydarzenia
if (diffDays > 0) {
...
document.addEventListener("DOMContentLoaded", () => {
    // Śledzenie wyświetlenia instrukcji dla Androida
    const androidInstructions = document.getElementById("android-instructions");
    if (androidInstructions) {
        androidInstructions.addEventListener("mouseover", () => {
            var _paq = window._paq || [];
            _paq.push(['trackEvent', 'PWA', 'ViewInstructions', 'Android']);
            console.log("Matomo: Wyświetlono instrukcje dla Androida");
        });
    }

    // Śledzenie wyświetlenia instrukcji dla iOS
    const iosInstructions = document.getElementById("ios-instructions");
    if (iosInstructions) {
        iosInstructions.addEventListener("mouseover", () => {
            var _paq = window._paq || [];
            _paq.push(['trackEvent', 'PWA', 'ViewInstructions', 'iOS']);
            console.log("Matomo: Wyświetlono instrukcje dla iOS");
        });
    }

    // Opcjonalne: śledzenie kliknięcia w "Dodaj do ekranu głównego"
    const addToHomeScreenButtons = document.querySelectorAll(".add-to-home-btn");
    addToHomeScreenButtons.forEach(button => {
        button.addEventListener("click", () => {
            var _paq = window._paq || [];
            _paq.push(['trackEvent', 'PWA', 'AddToHomeScreen', 'Kliknięcie']);
            console.log("Matomo: Kliknięto Dodaj do ekranu głównego");
        });
    });
});
...
window.addEventListener("beforeinstallprompt", (e) => {
    ...
    var _paq = window._paq || [];
    _paq.push(['trackEvent', 'PWA', 'PromptDisplayed', 'Android']);
    console.log("Matomo: Wyświetlono prompt Dodaj do ekranu głównego (Android)");
});
